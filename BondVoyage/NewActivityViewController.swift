...
class NewActivityViewController: UIViewController, CLLocationManagerDelegate, GMSMapViewDelegate {
    ...
    @IBAction func didClickRequest(sender: UIButton) {
        ...
        ActivityRequest.createActivity(self.selectedCategories!, location: self.currentLocation!, locationString: self.inputCity.text, aboutSelf: nil, aboutOthers: []) { (result, error) -> Void in
            if error != nil {
